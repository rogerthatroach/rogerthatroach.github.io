...
export function GET(): Response {
  ...
  const approachTaxonomy = {
    ...
    retrieval:
      'AI/LLM Drafting Platform and WorkforceAnalytics use an LLM-as-router (the LLM does keyword/fuzzy matching to scope a subset) followed by dense RAG — not BM25, sparse, hybrid, or RRF.',
    fineTuning:
    ...
    dataBoundary:
      'In WorkforceAnalytics the LLM never touches operational data by construction — GPT is used only for parse / route / metadata extraction / synthesis; deterministic code handles all data access.',
  };
