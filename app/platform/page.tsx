...
const STACK: { title: string; body: React.ReactNode }[] = [
  {
    ...
    body: (
      <>
        ...
        (approved foundation model &mdash; approved foundation model in production). Multi-vendor
        by construction; services request capabilities, ModelGateway picks the
        route.
      </>
    ),
  },
  ...
  },
  {
    ...
    body: (
      <>
        ...
        FundingRequest runs production RAG on this stack: field-group
        classifier picks groups, two-stage retrieval pulls top-K
        passages from <span className="font-mono">pgvector</span>,
        parallel extraction agents run against approved foundation model (via
        ModelGateway), results merge as dict-union, every typed MCP tool
...
const ACTIVITIES: { lead: string; body: string }[] = [
  {
    ...
    body: 'Through ModelGateway (vendor-agnostic LLM access, rate limit, audit), on OCP (containerized, observable), on Postgres (state + audit + embeddings).',
  },
  {
    lead: 'Define the contract.',
    body: 'What does "an AI service at RBC" look like? Typed APIs, structured logs, audit-by-default, multi-vendor LLM access via ModelGateway, RAG via pgvector. The architectural pattern is the artifact.',
  },
  {
...
const CAPABILITIES: { surface: string; evidence: React.ReactNode }[] = [
  ...
  },
  {
    ...
    evidence: (
      <>
        ...
        ,{' '}
        <a
          href="https://github.com/rogerthatroach/funding-request-drafting-multi-agent-retrieval"
          target="_blank"
  ...
  },
  {
    ...
    evidence: (
      <>
        ...
        ,{' '}
        <a
          href="https://github.com/rogerthatroach/workforce-analytics-llm-as-router"
          target="_blank"
          rel="noopener noreferrer"
          className="text-accent underline underline-offset-4 hover:text-text-primary"
        >
  ...
  },
  {
    ...
    evidence: (
      <>
        Typed MCP tool registry &rarr; Postgres audit log (FundingRequest);
        provenance algebra (
        <Link
  ...
  },
  {
    ...
    evidence: (
      <>
        ...
        ,{' '}
        <a
          href="https://github.com/rogerthatroach/financialBenchmarking-guarded-text-to-sql"
          target="_blank"
          rel="noopener noreferrer"
          className="text-accent underline underline-offset-4 hover:text-text-primary"
        >
...
const REFERENCE_IMPLEMENTATIONS: {
  ...
  {
    name: 'financialBenchmarking-guarded-text-to-sql',
    url: 'https://github.com/rogerthatroach/financialBenchmarking-guarded-text-to-sql',
    backs: 'FinancialBenchmarking text-to-SQL — calibration · AST validation · template-bank fallback',
  },
  {
  ...
  },
  {
    ...
    backs: 'WorkforceAnalytics — LLM-as-router + 5-stage permission cascade',
  },
  {
    ...
    backs: 'FundingRequest — two-stage retrieval + N parallel extraction + coverage loop',
  },
];
...
export const metadata: Metadata = {
  ...
  description:
    'The platform stack underneath the systems — ModelGateway LLM gateway, OpenShift runtime, PostgreSQL + pgvector, Postgres-backed audit. What I do at this layer, mapped to evidence.',
      ...
      >
        ...
        <div>
          ...
          <p className="mt-3 text-sm leading-relaxed text-text-secondary">
            The systems on the home page &mdash; FundingRequest, WorkforceAnalytics,
          ...
          <p className="mt-6 rounded-lg border border-border-subtle bg-surface/30 p-4 text-sm leading-relaxed text-text-secondary">
            ModelGateway is owned and operated by another RBC team. The systems
            documented here integrate through it; their architectural shape
            is downstream of what ModelGateway makes easy or hard.
          </p>
