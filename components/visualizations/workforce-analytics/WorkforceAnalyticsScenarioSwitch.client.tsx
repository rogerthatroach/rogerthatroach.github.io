...
import type {
  WorkforceAnalyticsCaseOverviewContent,
  WorkforceAnalyticsOverviewScenario,
} from '@/data/visualizations/workforceAnalytics';

interface WorkforceAnalyticsScenarioSwitchProps {
...
function ScenarioFlow({
  ...
  content: WorkforceAnalyticsCaseOverviewContent;
  scenario: WorkforceAnalyticsOverviewScenario;
}) {
  return (
    <div className="mt-5">
...
export default function WorkforceAnalyticsScenarioSwitch({ content }: WorkforceAnalyticsScenarioSwitchProps) {
  const [selectedId, setSelectedId] = useState<WorkforceAnalyticsOverviewScenario['id']>('simple');
  const panelId = useId();
