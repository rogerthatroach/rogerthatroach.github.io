...
approved foundation model, exhibited two pathological failure modes on the disambiguation step:
...
approved foundation model closed both gaps to within tolerance on a held-out evaluation set of
...
**Factor 3: Model readiness.** An earlier prototype with approved foundation model had
demonstrated the architecture but failed the calibration bar Requirement 3
demands (see §6.4). When approved foundation model cleared that bar on a held-out evaluation,
...
Working code, synthetic data, and reproducible plots: [`rogerthatroach/financialBenchmarking-guarded-text-to-sql`](https://github.com/rogerthatroach/financialBenchmarking-guarded-text-to-sql).
