import {
  ...
  FINANCIAL_BENCHMARKING_V2_BUILD_TIME,
...
const PROJECTS_CHRONOLOGICAL: Project[] = [
  ...
  {
    id: 'financialBenchmarking',
    title: 'FinancialBenchmarking v2',
    ...
    heroMetric: { value: FINANCIAL_BENCHMARKING_V2_BUILD_TIME, label: 'v2 solo build' },
    caption:
      'Canadian Big 6 bank peer-benchmarking engine, conceived and built end-to-end. v1 (Sr DS, solo): automated extraction and matching despite quarterly Supplementary Financial Package schema shifts, breaking the long-standing peer-analysis bottleneck. v2 (Lead, 2-week solo sprint, run in parallel with WorkforceAnalytics and the Amplify intern program): multi-stage RAG with multi-gate query parsing across bank, parameter, platform, and time-period, plus a text-to-SQL layer over rich KPI metadata and embeddings. v2 productionalized by direct report. CFO One RBC Team Award (2025) for v1.',
    description:
    ...
    deepDivePath: '/projects/financialBenchmarking',
  },
  {
    ...
    stack: ['approved foundation model', 'Custom Python router', 'Cython compute', 'Postgres', 'EPM'],
    heroMetric: { value: WORKFORCE_ANALYTICS_COST_CENTRES, label: 'Cost centres' },
    caption:
    ...
    highlights: [
      'Two-wall architecture: approved foundation model handles gate / metadata extraction / answer / synthesis; deterministic Cython-compiled Python handles entitlement + compute. LLM never sees operational data.',
      '5 to 8 approved foundation model calls per query across four stages (1 gate + 3 parallel metadata extractions + 1-or-3 answer + 0-or-1 synthesis) — all on schemas and aggregates, never on rows. Cross-domain queries fan out to 3 parallel Answer-stage calls (Headcount / HR Costs / Open Positions) before a final synthesis.',
    ...
    palette: { primary: '#93c5fd', primaryLight: '#1e40af', glow: 'shadow-blue-500/20', bg: 'from-blue-500/5' },
    deepDivePath: '/projects/workforceAnalytics',
  },
  {
    ...
    caption:
      'The bank’s first true agentic AI platform. Single-agent governance envelope, LangGraph on Postgres, template-as-MCP-tool with decision-tree dialog, two-stage field-group retrieval, N parallel approved foundation model extraction calls merging as dict-union, coverage + follow-ups loop.',
    description:
      'Enterprise-wide drafting tool for project funding requests — a critical governance process. Acts as a personal assistant guiding users through each step, utilizing metadata, rules, policies, historical examples, and best practices.',
    highlights: [
      ...
      'Two-stage field-group retrieval: stage-1 picks relevant field groups, stage-2 retrieves top-10 chunks per group, custom compression feeds approved foundation model calls',
