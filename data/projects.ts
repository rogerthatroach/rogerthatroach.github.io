...
const PROJECTS_CHRONOLOGICAL: Project[] = [
  ...
  // Arc 4: Intelligent Systems (2024-present)
  {
    ...
    caption:
      'Canadian Big 6 bank peer-benchmarking engine. v1 (Sr DS, solo): built end-to-end to automate extraction and matching despite quarterly Supplementary Financial Package schema shifts, breaking the long-standing peer-analysis bottleneck. v2 (Lead, 2-week concurrent refactor sprint, run in parallel with WorkforceAnalytics and the Amplify intern program): refactored the v1 benchmarking module into five stages for typed intent, embedding-based KPI candidate retrieval, accept-or-clarify resolution, reviewed query construction with bound values, and deterministic formatting. My direct report and the broader team integrated and productionalized it as FinancialBenchmarking v2. CFO One RBC Team Award (2025) for v1.',
    description:
  ...
  },
  {
    ...
    description:
      'Drafting tool for project funding requests — a critical governance process — launched across RBC’s full CFO Group in all geographies. Acts as a personal assistant guiding users through each step, utilizing metadata, rules, policies, historical examples, and best practices.',
    highlights: [
