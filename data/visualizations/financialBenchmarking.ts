...
export interface FinancialBenchmarkingFigureBase {
  id: string;
  title: string;
  thesis: string;
  headingLevel: 2 | 3 | 4;
...
export interface FinancialBenchmarkingCaseStage {
  id: string;
  number: string;
  name: string;
  job: string;
...
export interface FinancialBenchmarkingCaseScenario {
  id: string;
  label: string;
  question: string;
  summary: string;
...
export interface FinancialBenchmarkingCaseSpineContent extends FinancialBenchmarkingFigureBase {
  scenarioLabel: string;
  stageStatusLabels: Readonly<{ reached: string; notReached: string }>;
  stages: readonly FinancialBenchmarkingCaseStage[];
  scenarios: readonly FinancialBenchmarkingCaseScenario[];
  boundaryLabel: string;
  boundaryDetail: string;
}

export interface FinancialBenchmarkingMilestone {
  marker: string;
  title: string;
  detail: string;
}

export interface FinancialBenchmarkingMilestoneContent extends FinancialBenchmarkingFigureBase {
  milestones: readonly FinancialBenchmarkingMilestone[];
  continuityLabel: string;
  continuityDetail: string;
}

export interface FinancialBenchmarkingStructuralDiffRow {
  aspect: string;
  before: string;
  after: string;
}

export interface FinancialBenchmarkingStructuralDiffContent extends FinancialBenchmarkingFigureBase {
  sharedLabel: string;
  sharedDetail: string;
  columnLabels: readonly [string, string, string];
  rows: readonly FinancialBenchmarkingStructuralDiffRow[];
  continuityLabel: string;
  continuityDetail: string;
}

export interface FinancialBenchmarkingDecisionStage {
  id: string;
  number: string;
  name: string;
  job: string;
...
export interface FinancialBenchmarkingDecision {
  number: string;
...
export interface FinancialBenchmarkingDecisionSpineContent extends FinancialBenchmarkingFigureBase {
  stages: readonly FinancialBenchmarkingDecisionStage[];
  decisions: readonly FinancialBenchmarkingDecision[];
  columnLabels: readonly [string, string, string];
  operatingRuleLabel: string;
  operatingRuleDetail: string;
}

export interface FinancialBenchmarkingWalkthroughStep {
  title: string;
  content: string;
  caption?: string;
}

export interface FinancialBenchmarkingFormalStage {
  id: string;
  number: string;
  name: string;
  input: string;
...
export interface FinancialBenchmarkingBoundary {
  afterStageId: string;
  label: string;
  detail: string;
}

export interface FinancialBenchmarkingFormalOverviewContent extends FinancialBenchmarkingFigureBase {
  columnLabels: readonly [string, string, string, string];
  stages: readonly FinancialBenchmarkingFormalStage[];
  boundaries: readonly FinancialBenchmarkingBoundary[];
  legend: readonly { label: string; detail: string }[];
}

export interface FinancialBenchmarkingCandidate {
  label: string;
  detail: string;
}

export interface FinancialBenchmarkingCandidateRoute {
  id: string;
  countLabel: string;
  title: string;
  request: string;
  candidates: readonly FinancialBenchmarkingCandidate[];
  decision: string;
  outcome: string;
}

export interface FinancialBenchmarkingCandidateAmbiguityContent extends FinancialBenchmarkingFigureBase {
  routes: readonly FinancialBenchmarkingCandidateRoute[];
...
export interface FinancialBenchmarkingValidatorInput {
  label: string;
  detail: string;
}

export interface FinancialBenchmarkingValidatorCheck {
  number: string;
  title: string;
  detail: string;
  failure: string;
...
export interface FinancialBenchmarkingValidatorOutcome {
  condition: string;
  title: string;
  detail: string;
}

export interface FinancialBenchmarkingValidatorContent extends FinancialBenchmarkingFigureBase {
  inputLabel: string;
  missingControlLabel: string;
  inputs: readonly FinancialBenchmarkingValidatorInput[];
  checks: readonly FinancialBenchmarkingValidatorCheck[];
  pass: FinancialBenchmarkingValidatorOutcome;
  reject: FinancialBenchmarkingValidatorOutcome;
}

export interface FinancialBenchmarkingTraceStep {
  stage: string;
  state: string;
  action: string;
  evidence: string;
...
export interface FinancialBenchmarkingTrace {
  id: string;
  label: string;
  question: string;
  summary: string;
  steps: readonly FinancialBenchmarkingTraceStep[];
  outcomeLabel: string;
  outcomeDetail: string;
}

export interface FinancialBenchmarkingExecutionTraceContent extends FinancialBenchmarkingFigureBase {
  scenarioLabel: string;
  fieldLabels: readonly [string, string, string];
  statusLabels: Readonly<Record<FinancialBenchmarkingTraceStep['status'], string>>;
  traces: readonly FinancialBenchmarkingTrace[];
...
export const FINANCIAL_BENCHMARKING_PRACTITIONER_WALKTHROUGH: FinancialBenchmarkingWalkthroughStep[] = [
  {
...
[
  FINANCIAL_BENCHMARKING_BUILDER_MILESTONES,
  FINANCIAL_BENCHMARKING_BUILDER_DIFF,
  FINANCIAL_BENCHMARKING_FORMAL_VALIDATOR,
  FINANCIAL_BENCHMARKING_FORMAL_TRACES,
].forEach((spec) => validateVisualizationSpec(spec));

validateVisualizationSpec(FINANCIAL_BENCHMARKING_CASE_SPINE, {
  relationships: FINANCIAL_BENCHMARKING_CASE_SPINE.scenarios.map((scenario) => ({
    name: `scenarios[id=${scenario.id}].activeStageIds`,
    referencedIds: scenario.activeStageIds,
    targetIds: FINANCIAL_BENCHMARKING_CASE_SPINE.stages.map((stage) => stage.id),
  })),
});

validateVisualizationSpec(FINANCIAL_BENCHMARKING_PRACTITIONER_DECISIONS, {
  additionalAuthoredArrays: [
    { name: 'walkthrough', values: FINANCIAL_BENCHMARKING_PRACTITIONER_WALKTHROUGH },
  ],
  relationships: FINANCIAL_BENCHMARKING_PRACTITIONER_DECISIONS.decisions.map((decision) => ({
    name: `decisions[${decision.number}].stageIds`,
    referencedIds: decision.stageIds,
    targetIds: FINANCIAL_BENCHMARKING_PRACTITIONER_DECISIONS.stages.map((stage) => stage.id),
  })),
});

validateVisualizationSpec(FINANCIAL_BENCHMARKING_FORMAL_OVERVIEW, {
  relationships: FINANCIAL_BENCHMARKING_FORMAL_OVERVIEW.boundaries.map((boundary) => ({
    name: `boundaries[after=${boundary.afterStageId}]`,
    referencedIds: [boundary.afterStageId],
    targetIds: FINANCIAL_BENCHMARKING_FORMAL_OVERVIEW.stages.map((stage) => stage.id),
  })),
});

validateVisualizationSpec(FINANCIAL_BENCHMARKING_FORMAL_CANDIDATES, {
