...
export const CASE_STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    tldr: {
      ...
      impact:
        'Shipped to production in 2 weeks — while simultaneously running WorkforceAnalytics development and the summer intern program. v1 had earned the 2025 CFO One RBC Team Award; v2 inherited the trust that made the sprint possible.',
    },
    sections: {
      context:
  ...
  },
  {
    ...
    tldr: {
      ...
      decision:
        'Two-wall architecture. approved foundation model handles parse / route / metadata extraction / synthesis only — never touches operational data. Between the walls, Cython-compiled Python runs event-level ins-outs math in milliseconds over ~40K leaf-level events, with the permission-to-SQL entitlement cascade applied before compute. In-scope answers either go straight to a single synthesis call or fan out to up to 3 parallel final subagents (entitlement / Headcount / Open Positions) before combining.',
      impact:
    ...
    sections: {
      ...
      challenge:
        'Three simultaneous hard problems. (1) The factorial wall — arbitrary-combination queries across 40K leaf-level events × 9K rollups × 60K geographies look combinatorially impossible at dynamic response times. Previous attempts scoped the feature out. (2) Security — enterprise permission entitlements had to translate from cube-level permissions into SQL-level access controls by construction, not post-hoc filters. (3) Trust — this is CFO-grade analytics; non-determinism anywhere in the data path is disqualifying. LLMs had to be on one side of a hard line, data on the other, with no leakage.',
      optionsConsidered: [
        {
  ...
  },
  {
    ...
    sections: {
      ...
      stakeholders:
        'CFO Group leadership (sponsor), enterprise stakeholders across the bank (PAR authors in every department), Director AI (governance and priority), summer interns (original POC developers), engineering services partner (co-delivery on productionization).',
      challenge:
      ...
      impact:
        'Transformed a manual, multi-week governance process into a guided drafting session with every field cited back to its source. Pilot launched April 2026 with the first wave of PAR authors; enterprise rollout expands across RBC business lines through Q2/Q3 2026. Beyond the direct drafting win, FundingRequest is the reference architecture for agentic AI at the bank — the MCP tool registry, Postgres backbone, and single-agent envelope are the substrate v2 (multi-agent "skills" framework) is being built on. v1 is the part of v2 that already passed review.',
      inProduction:
      ...
      lessonsLearned:
        'Two lessons, one technical and one organizational. Technical: in regulated environments, the first agentic deployment is not about maximum capability — it is about building the envelope in a way that the next envelope (larger) inherits without rewrite. Every decision — the MCP tool registry, the Postgres backbone, the field-group taxonomy — was picked so that v2 skills compose over the same substrate. v1 is not a throwaway; v1 is the part of v2 that already passed governance review. Organizational: great ideas can come from anywhere (a summer intern proposed FundingRequest) — the leadership job is to recognize potential early, provide the architectural vision that lets the idea scale, and create the environment where productionization doesn\'t kill momentum. Most AI POCs die in the transition to enterprise product; the fix is to plan that transition before you write the POC.',
    },
