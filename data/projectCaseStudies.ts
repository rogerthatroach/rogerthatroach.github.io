...
export const CASE_STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    tldr: {
      ...
      decision:
        'Two-wall architecture. GPT-4.1 handles parse / route / metadata extraction / synthesis only — never touches operational data. Between the walls, Cython-compiled Python runs event-level ins-outs math in milliseconds over ~40K transits, with the EPM-to-SQL entitlement chain applied before compute. In-scope answers either go straight to a single synthesis call or fan out to up to 3 parallel final subagents (EPM / Headcount / Open Positions) before combining.',
      impact:
  ...
  },
  {
    ...
    sections: {
      ...
      lessonsLearned:
        'Two lessons, one technical and one organizational. Technical: in regulated environments, the first agentic deployment is not about maximum capability — it is about building the envelope in a way that the next envelope (larger) inherits without rewrite. Every decision — the MCP tool registry, the Postgres backbone, the field-group taxonomy — was picked so that v2 skills compose over the same substrate. v1 is not a throwaway; v1 is the part of v2 that already passed governance review. Organizational: great ideas can come from anywhere (an Amplify intern proposed PAR Assist) — the leadership job is to recognize potential early, provide the architectural vision that lets the idea scale, and create the environment where productionization doesn\'t kill momentum. Most AI POCs die in the transition to enterprise product; the fix is to plan that transition before you write the POC.',
    },
