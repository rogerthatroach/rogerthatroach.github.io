...
export const CASE_STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    sequencing:
      'Commodity Tax was the cascade origin. When I joined the CFO Group in 2022, nobody had asked me to build AI — I was hired to automate a tax process. Delivering that in under a year, with dashboards the CFO could audit, earned the credibility to propose FinancialBenchmarking v1 (Big 6 bank benchmarking, 2024), then FinancialBenchmarking v2 (AI-native rewrite, 2 weeks, 2024), then WorkforceAnalytics (production 2025), then conceive AI/LLM Drafting Platform from an intern POC (shipped April 2026). Each project underwrote the next one\'s scope. The Commodity Tax wasn\'t about tax — it was about proving I could deliver production systems in regulated finance before asking for license to build something ambitious.',
    blogPostSlug: 'commodity-tax-cfo-trust',
    leadershipCallout:
      'This was my first project at RBC — and I treated it as an audition. By choosing Tableau as the transparency layer (not just an output), I gave skeptical finance stakeholders visibility into every step of the automation. The resulting trust didn\'t just deliver Commodity Tax — it opened the door for FinancialBenchmarking, WorkforceAnalytics, and every AI initiative that followed. The months-to-90-minutes metric became the team\'s calling card with CFO leadership.',
    sections: {
      context:
  ...
  },
  {
    ...
    tldr: {
      ...
      impact:
        'Shipped to production in 2 weeks — while simultaneously running WorkforceAnalytics development and the Amplify internship program. 2025 CFO One RBC Team Award.',
    },
    sections: {
      context:
        'RBC\'s CFO Group needed to benchmark financial KPIs against the Big 6 Canadian banks using Supplementary Financial Packages — publicly available but complex financial disclosures. FinancialBenchmarking v1 was a rules-based benchmarking engine I\'d built and productionized earlier. v2 was the AI-native evolution: natural language queries to validated SQL, with embeddings-based KPI disambiguation.',
      myRole:
        'Sole designer and builder of FinancialBenchmarking v2. I designed the five-stage pipeline architecture, implemented it, and shipped it to production — all within two weeks, while simultaneously leading WorkforceAnalytics development and the Amplify internship program. This was a deliberate sprint: months of research and brainstorming crystallized into two weeks of focused execution.',
      stakeholders:
      ...
      optionsConsidered: [
        {
          option: 'Extend FinancialBenchmarking v1 with LLM layer on top',
    ...
    companionBlogPostSlug: 'financial-benchmarking-refactor-velocity',
  },
  {
    ...
    tldr: {
      problem:
        'CFO Group financial questions — "headcount by division, crossed with open positions, crossed with tenure" — bounced through days of email. No interactive system could answer dynamic arbitrary-combination queries across ~40K transits with entitlement controls. The dynamic ins-outs question (who joined / left / moved between any two arbitrary groups) was off the table — every prior attempt hit the factorial wall.',
      decision:
        'Two-wall architecture. approved foundation model handles parse / route / metadata extraction / synthesis only — never touches operational data. Between the walls, Cython-compiled Python runs event-level ins-outs math in milliseconds over ~40K transits, with the EPM-to-SQL entitlement chain applied before compute. In-scope answers either go straight to a single synthesis call or fan out to up to 3 parallel final subagents (EPM / Headcount / Open Positions) before combining.',
      impact:
    ...
    sections: {
      context:
        'RBC\'s CFO Group needed a single platform for financial insights — headcount analytics, HR costs, open positions — serving three delivery channels: interactive dashboard, chatbot, and inbox-ready HTML reports. The data spans ~40,000 employee transits (leaves) queryable in any combination, rolling up through ~9,000 parent nodes, ~80,000 business-hierarchy nodes, and ~60,000 geography-hierarchy nodes — with strict entitlement controls (users see only what they\'re authorized to access).',
      myRole:
        'Architect, lead developer, and product visionary. Conceived the "CFO-ready" vision — at-a-glance, trustworthy insights delivered to the inbox with drill-through across domains. Designed the two-wall architecture, built the EPM-to-SQL entitlement chain, wrote the event-level ins-outs math with Cython-compiled Python, and led cross-functional delivery with GFT (Global Functions Technology) while writing a significant portion of the codebase. ~70% hands-on.',
      stakeholders:
        'CFO Group leadership (executive consumers), finance analysts and HR teams (daily users), GFT engineering peers (co-delivery on frontend and infrastructure), Director AI (governance and priority), 1 Senior AI Scientist direct report contributing to development, rotating Amplify interns.',
      challenge:
        'Three simultaneous hard problems. (1) The factorial wall — arbitrary-combination queries across 40K leaves × 9K rollups × 60K geographies look combinatorially impossible at dynamic response times. Previous attempts scoped the feature out. (2) Security — EPM (Enterprise Performance Management) entitlements had to translate from cube-level permissions into SQL-level access controls by construction, not post-hoc filters. (3) Trust — this is CFO-grade analytics; non-determinism anywhere in the data path is disqualifying. LLMs had to be on one side of a hard line, data on the other, with no leakage.',
      optionsConsidered: [
        {
  ...
  },
  {
    ...
    tldr: {
      problem:
        'RBC\'s project funding request process — governance for every major bank initiative — took weeks to draft. Fragmented templates, contradictory policies, and institutional knowledge locked in people\'s heads.',
      decision:
        'First true agentic framework approved for production at the bank, built inside a single-agent governance envelope: LangGraph on a Postgres backbone, template-as-MCP-tool with decision-tree dialog, two-stage field-group retrieval with custom compression, N parallel approved foundation model extraction calls merging as a dictionary union, coverage + follow-ups loop.',
      impact:
    ...
    sections: {
      context:
        'project funding requests (PARs) are a critical governance process at RBC — every major initiative requires one, and drafting them is a complex, time-consuming process involving metadata, policies, historical examples, and institutional knowledge. The idea originated from the 2025 Amplify internship program, where an intern proposed a tool to help with PAR drafting. I saw the potential to scale it from a prototype to an enterprise-wide platform — and to use AI/LLM Drafting Platform as the bank\'s first true agentic AI deployment, setting the production-architecture pattern every subsequent agentic system would inherit.',
      myRole:
      ...
      optionsConsidered: [
        ...
        },
        {
          ...
          prosAndCons: 'LangGraph picked for maturity (stable, active, production-deployed). MCP tools make every action a typed, logged contract — audit is structural, not aspirational. Two-stage retrieval organizes knowledge by the target fields the LLM extracts toward, not by source document. N parallel approved foundation model calls (one per relevant field group, ≤20 fields each with rich metadata) gives multi-agent-like parallelism inside a single-agent envelope. Postgres carries LangGraph checkpoints + logs + raw/mapped content + pgvector embeddings + audit — one store, one provenance graph.',
          chosen: true,
        },
      ],
      decision:
        'PAR drafting is a stateful, branching workflow over a structured form with regulatory audit requirements. LangGraph (picked for maturity over CrewAI/AutoGen, which we considered but didn\'t deeply evaluate) provides the graph engine and wires directly into a Postgres checkpointer for session durability. Template selection is itself an MCP tool with a decision-tree dialog — project type, spend tier, business line, regulatory exposure — returning a typed template-id with confidence and rationale. Retrieval reshapes the knowledge base around the target: field groups aggregate logically related fields across all PAR templates; stage-1 picks relevant groups for the session, stage-2 retrieves top-10 chunks per group, custom compression fits the payload into a approved foundation model system prompt carrying up to 20 fields of rich metadata. The extraction layer fans out to N parallel calls (one per relevant group), each returning a JSON dictionary with guardrails and few-shot good/bad examples scoped to its group. The merge is a deterministic dictionary union. A coverage analyzer checks the merged state against guidelines + examples, surfaces intelligent follow-ups, and loops the session back through a clarify node when answers are still open.',
      implementation:
        'LangGraph graph orchestrates the session (intake → template → retrieve → extract → merge → coverage → respond), with conditional edges for loops. Template selection, retrieval, compression, extraction, merge, and coverage are all MCP tools — every action typed, logged, and dispatched through the graph engine. Ingestion handles pptx, docx, pdf, txt, and images (OCR via approved foundation model). Field groups are a taxonomy over the hundreds of fields across PAR templates; retrieval runs two-stage similarity search with custom compression on top-10 chunks per group. Parallelism = relevant-group count: up to N approved foundation model calls fire at once, each carrying ≤20 fields + guardrails + few-shot good/bad examples; each returns JSON. Merge is a dict union (groups disjoint by construction). Coverage analyzer gates the draft — if it finds open follow-ups, the session loops to a clarify node before returning to template/extraction. Postgres (with pgvector for retrieval) is the single store: LangGraph checkpoints, tool-invocation logs, raw ingested content, mapped intermediates, embeddings, and the complete audit trail all land in one place.',
      impact:
        'Transformed a manual, multi-week governance process into a guided drafting session with every field cited back to its source. Pilot launched April 2026 with the first wave of PAR authors; enterprise rollout expands across RBC business lines through Q2/Q3 2026. Beyond the direct drafting win, AI/LLM Drafting Platform is the reference architecture for agentic AI at the bank — the MCP tool registry, Postgres backbone, and single-agent envelope are the substrate v2 (multi-agent "skills" framework) is being built on. v1 is the part of v2 that already passed review.',
      inProduction:
      ...
      lessonsLearned:
        'Two lessons, one technical and one organizational. Technical: in regulated environments, the first agentic deployment is not about maximum capability — it is about building the envelope in a way that the next envelope (larger) inherits without rewrite. Every decision — the MCP tool registry, the Postgres backbone, the field-group taxonomy — was picked so that v2 skills compose over the same substrate. v1 is not a throwaway; v1 is the part of v2 that already passed governance review. Organizational: great ideas can come from anywhere (an Amplify intern proposed AI/LLM Drafting Platform) — the leadership job is to recognize potential early, provide the architectural vision that lets the idea scale, and create the environment where productionization doesn\'t kill momentum. Most AI POCs die in the transition to enterprise product; the fix is to plan that transition before you write the POC.',
    },
