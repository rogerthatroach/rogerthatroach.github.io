...
export const CASE_STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    tldr: {
      ...
      impact:
        'Built the v2 benchmarking module in a focused 2-week sprint — while simultaneously running WorkforceAnalytics development and the Amplify intern program — then handed it to my direct report, who incorporated it into FinancialBenchmarking v2 with the broader team. v1 had earned the 2025 CFO One RBC Team Award; v2 inherited the trust that made the sprint possible.',
    },
    sections: {
      context:
      ...
      myRole:
        'Designed and built the FinancialBenchmarking v2 benchmarking module — the five-stage pipeline architecture, implemented end-to-end — in a focused two-week sprint while simultaneously leading WorkforceAnalytics development and the Amplify intern program, then handed it to my direct report, who collaborated with the broader team to incorporate it into FinancialBenchmarking v2. A deliberate sprint: months of research and brainstorming crystallized into two weeks of focused execution.',
      stakeholders:
  ...
  },
  {
    ...
    tldr: {
      ...
      decision:
        'Two-wall architecture. approved foundation model handles parse / route / metadata extraction / synthesis only — never touches operational data. Between the walls, Cython-compiled Python runs event-level ins-outs math in milliseconds over ~40K leaf-level cost centres, with the permission-to-SQL entitlement cascade applied before compute. In-scope answers either go straight to a single synthesis call or fan out to up to 3 parallel final subagents (entitlement / Headcount / Open Positions) before combining.',
      impact:
  ...
  },
  {
    ...
    sections: {
      ...
      impact:
        'Transformed a manual, multi-week governance process into a guided drafting session with every field cited back to its source. Pilot launched April 2026 with the first wave of PAR authors; deployed bank-wide May 2026 across RBC business lines. Beyond the direct drafting win, AI/LLM Drafting Platform is the reference architecture for agentic AI at the bank — the MCP tool registry, Postgres backbone, and single-agent envelope are the substrate v2 (multi-agent "skills" framework) is being built on. v1 is the part of v2 that already passed review.',
      inProduction:
