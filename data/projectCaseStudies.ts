...
export const CASE_STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    sections: {
      ...
      myRole:
        'Refactored the FinancialBenchmarking v1 benchmarking module into a five-stage v2 pipeline in a focused two-week sprint while simultaneously leading WorkforceAnalytics development and the Amplify intern program, then handed it to my direct report, who collaborated with the broader team to integrate and productionalize it as FinancialBenchmarking v2. A deliberate sprint: months of research and brainstorming crystallized into two weeks of focused execution.',
      stakeholders:
      ...
      impact:
        'Refactored the v1 benchmarking module into the production text-to-SQL architecture in two weeks; my direct report then integrated and productionalized it with the broader team as FinancialBenchmarking v2. Reviewed templates, parameter binding, schema checks, confidence-gated clarification, and stage-level tests constrain the failure surface; they remain dependent on configuration, validation coverage, and model calibration. The focused refactor ran concurrently with WorkforceAnalytics productionization and the Amplify intern program. Its speed came from a shelved-and-resumed architecture—an earlier model had failed the calibration bar, while a later model cleared the held-out evaluation—plus the operational trust v1 had already earned. The 2025 CFO One RBC Team Award recognized v1\u2019s productionization, the precondition for v2\u2019s mandate.',
      inProduction:
  ...
  },
  {
    ...
    tldr: {
      problem:
        'RBC\'s project funding request process is a manual, iterative governance workflow shaped by fragmented templates, sometimes-conflicting policies, and institutional knowledge held by experienced authors.',
      decision:
