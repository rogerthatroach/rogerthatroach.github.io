...
export const GLOSSARY: Record<string, string> = {
  ...
  'AI/LLM Drafting Platform':
    'The first true agentic AI platform approved for production at the bank, guiding project funding request (PAR) drafting. Pilot launched April 2026; full CFO Group launch across all geographies followed in May 2026.',
  'WorkforceAnalytics':
  ...
  'FinancialBenchmarking':
    'Strategic peer-benchmarking engine over Big 6 Canadian banks\' Supplementary Financial Packages. v1 (Sr DS period): solo end-to-end build that automated extraction and matching despite quarterly SFP schema shifts, the long-standing bottleneck blocking timely peer analysis. v2 (Lead, 2025): 2-week concurrent refactor of the v1 benchmarking module into multi-stage RAG with multi-gate query parsing across bank / parameter / platform / time-period plus text-to-SQL, run in parallel with WorkforceAnalytics and the Amplify intern program; integrated and productionalized by my direct report with the broader team. 2025 CFO One RBC Team Award for v1.',
  'Commodity Tax':
  ...
  'summer intern program':
    'RBC Amplify internship program. I led the 2025 cohort end-to-end: 4 interns across PAR drafting and adjacent projects. The AI/LLM Drafting Platform concept was my vision, given to the interns as an ideation exercise to explore the problem space; the production platform was conceived, architected, and built end-to-end thereafter.',

  // ── Organizations ──
  'engineering services partner': 'Cross-functional engineering peers — senior + junior — on WorkforceAnalytics delivery.',
  'CFO Group':
  ...
  // ── Technical concepts ──
  LangGraph:
    'Graph-based LLM orchestration library. Picked for AI/LLM Drafting Platform after evaluation for its maturity and fit with a conditional-branching workflow — template selection → field-group retrieval → extraction → coverage loops back on open follow-ups.',
  MCP: 'Model Context Protocol — a protocol for connecting model applications to tools and context through defined interfaces. AI/LLM Drafting Platform routes its core workflow actions through typed MCP tools and records structured dispatch metadata for review.',
  pgvector:
    'PostgreSQL vector-similarity extension. It supports AI/LLM Drafting Platform\'s vector-backed retrieval alongside retained workflow state and structured trace records.',
  'field-group retrieval':
    'AI/LLM Drafting Platform\'s two-stage retrieval pattern. Stage 1 picks which logically related field groups are relevant to the session. Stage 2 retrieves a bounded candidate set within each group, then compression fits each group-scoped extraction payload to its context budget.',
  'single-agent envelope':
    'The governance constraint behind AI/LLM Drafting Platform v1 — the first true agentic AI platform approved for production at the bank. One agent, one scope, no multi-agent orchestration. Bounded concurrent group-scoped extraction calls provide specialized work through deterministic graph orchestration + MCP tools, inside the single-agent envelope.',
  'cost centre':
    'The most granular unit in WorkforceAnalytics\'s org model — one cost centre represents one or more teams (~40K in total). Cost centres are the shared leaves of an 18-level business-segment hierarchy with ~9K rollups and a separate geography hierarchy. Employee events and headcount net over those leaves in Cython-compiled ins-outs math.',
